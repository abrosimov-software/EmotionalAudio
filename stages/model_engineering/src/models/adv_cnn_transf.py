...
class CNNTransformerWithFeatures(nn.Module):
    ...
    CNN extracts features from spectrogram
    Transformer encoder processes and captures dependencies.
    additional features passed to final linear layer for classification.
    ...
    def __init__(self, model_cfg, output_size):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(**model_cfg["cnn"]["conv1"]),
            nn.ReLU(),
            nn.MaxPool2d(model_cfg["cnn"]["pool1_kernel_size"]),
            nn.Conv2d(**model_cfg["cnn"]["conv2"]),
            nn.ReLU(),
            nn.MaxPool2d(model_cfg["cnn"]["pool2_kernel_size"])
        )

        cnn_output_height = model_cfg["height"] // (2 * 2) 
        self.flattened_dim = model_cfg["cnn"]["conv2"]["out_channels"] * cnn_output_height

        transformer_layer = TransformerEncoderLayer(d_model=self.flattened_dim, **model_cfg["transformer"])
        self.transformer = TransformerEncoder(transformer_layer, num_layers=model_cfg["transformer"]["num_layers"])

        self.fc_features = nn.Linear(model_cfg["additional_features_size"], self.flattened_dim)
        self.fc = nn.Linear(self.flattened_dim * 2, output_size)
