...
class LSTMModel(nn.Module):
    ...
    Multiclass classification is done with the addition of extra features using CrossEntropyLoss.
    """
    def __init__(self, model_cfg, output_size):
        ...
        self.fc_additional = nn.Linear(model_cfg["additional_features_size"], model_cfg["lstm"]["hidden_size"])

        self.fc_final = nn.LazyLinear(output_size)

    def forward(self, x, additional_features):

        lstm_out, _ = self.lstm(x.permute(0, 2, 1)) # (batch, seq, feature) -> (batch, feature, seq)

        additional_features_proj = F.relu(self.fc_additional(additional_features))

        combined = torch.cat((lstm_out[:, -1, :], additional_features_proj), dim=1)
