...
class TemporalBlock(nn.Module):
    """
    Temporal block
    two convolutional layers, residual connection, optional downsampling.
    """
    def __init__(self, block_cfg):
        super(TemporalBlock, self).__init__()

        padding = (block_cfg["kernel_size"] - 1) * block_cfg["dilation"] // 2
        self.conv1 = nn.Conv1d(**{**block_cfg, "padding": padding})

        self.relu1 = nn.ReLU()
        
        self.conv2 = nn.Conv1d(**{**block_cfg, "padding": padding})

        self.relu2 = nn.ReLU()

        self.downsample = nn.Conv1d(block_cfg["in_channels"], block_cfg["out_channels"], 1) \
            if block_cfg["in_channels"] != block_cfg["out_channels"] else None
...
class TCNWithFeatures(nn.Module):
    ...
    combination with additional features for final classification.
    """
    def __init__(self, model_cfg, output_size):
        ...
        num_channels = [model_cfg["input_height"]] + [model_cfg["tcn"]["block_out_channels"]] * model_cfg["num_levels"]
        ...
        for i in range(model_cfg["num_levels"]):
            ...
            block_cfg = {
                ...
                "kernel_size": model_cfg["tcn"]["kernel_size"],
                "stride": model_cfg["tcn"]["stride"],
                "dilation": dilation_size
            }
            layers.append(TemporalBlock(block_cfg))
        self.tcn = nn.Sequential(*layers)
        
        self.fc_features = nn.Linear(model_cfg["additional_features_size"], model_cfg["tcn"]["block_out_channels"])

        self.fc_final = nn.Linear(model_cfg["tcn"]["block_out_channels"] * 2, output_size)
